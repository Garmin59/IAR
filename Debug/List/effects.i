...
void effects_proc (void)
{
  if (get_message (1)) // каждые 20 мс после вывода в массив таймера
  {
    effect2 ();
  }
  sum_layers ();
...
void light_shadow (pixel_t *layer, light_t *light)
{
  ...
  for (i = 0; i < 50; i++) // для всех пикселей в ленте
  {
...
void effect1 (void)
{
  const static color_t  p_RED[1]    = {255, 0, 0};  //, CIAN, GREEN, YELLOW, ORANGE, RED}; // в светлячке 3 пикселя
  ...
  const static color_t  p_YELLOW[1] = {255, 80, 0};  //, RED, CIAN};  //, GREEN, YELLOW, ORANGE, RED}; // в светлячке 3 пикселя
